...
from computeEvaltool.config import TaskConfig, parse_task_config
from computeEvaltool.constants import DataCollection, EvalBackend
from computeEvaltool.utils.io_utils import OutputsStructure
...
if TYPE_CHECKING:
    from computeEvaltool.models import LocalModel
...
def setup_work_directory(task_cfg: TaskConfig, run_time: str):
    ...
    if task_cfg.eval_backend == EvalBackend.OPEN_COMPASS:
        ...
        from computeEvaltool.backend.rag_eval import Tools
...
def get_backend_manager_class(eval_backend: EvalBackend):
    ...
    if eval_backend == EvalBackend.OPEN_COMPASS:
        from computeEvaltool.backend.opencompass import OpenCompassBackendManager
        return OpenCompassBackendManager
    elif eval_backend == EvalBackend.VLM_EVAL_KIT:
        from computeEvaltool.backend.vlm_eval_kit import VLMEvalKitBackendManager
        return VLMEvalKitBackendManager
    elif eval_backend == EvalBackend.RAG_EVAL:
        from computeEvaltool.backend.rag_eval import RAGEvalBackendManager
...
def evaluate_model(task_cfg: TaskConfig, outputs: OutputsStructure) -> dict:
    ...
    from computeEvaltool.models import get_local_model
    from computeEvaltool.report import gen_table
...
def create_evaluator(task_cfg: TaskConfig, dataset_name: str, outputs: OutputsStructure, base_model: 'LocalModel'):
    ...
    from computeEvaltool.benchmarks import Benchmark, BenchmarkMeta
    from computeEvaltool.evaluator import Evaluator
    from computeEvaltool.models import initialize_model_adapter

    benchmark: BenchmarkMeta = Benchmark.get(dataset_name)

    if dataset_name == DataCollection.NAME:
        # EvaluatorCollection is a collection of evaluators
        from computeEvaltool.collections import EvaluatorCollection
...
def main():
    from computeEvaltool.arguments import parse_args
    args = parse_args()
    run_task(args)
