...
#     "resnet-18": {
#         "batch_size": 256,
#         "client_concurrency_list": [16, 16, 16]  # 可以调整这个列表
#     },
#     "yolov10-s": {
#         "batch_size": 32,
#         "client_concurrency_list": [16, 16, 16]
#     },
#     "vit-large": {
#         "batch_size": 8,
#         "client_concurrency_list": [16, 16, 16]
...
#         print(f"目录 {dir_path} 不存在，生成随机噪音图用于测试...")
...
def load_images_from_dir(dir_path):
    ...
    image_paths = [os.path.join(dir_path, f) for f in os.listdir(dir_path) if os.path.splitext(f)[1].lower() in supported_extensions]
...
if __name__ == "__main__":
    ...
    if n_gpus == 0:
        logger.error("集群中没有 GPU，无法运行 Benchmark。")
        exit()
    ...
    for model in MODELS_TO_TEST:
        ...
        actors = [BenchmarkActor.remote(model, VIT_HF_ID, YOLO_WEIGHTS_ID) for _ in range(n_gpus)]
        logger.info(f"[bold white]启动 {model}[/bold white] | Batch={batch_size} | 并发配置={concurrency_list}")
        logger.info("[cyan]等待 Actors 初始化及预热...[/cyan]")
        time.sleep(5)

        for client_concurrency in concurrency_list:
            total_conc = batch_size * client_concurrency
            logger.info(f"[yellow]→ 测试配置[/]: Batch={batch_size}, Clients={client_concurrency}, 总并发={total_conc}")
            logger.info(f"[yellow]⏱️ 测试时长[/]: {TEST_DURATION_SECONDS} 秒")
            ...
            while time.perf_counter() - start_time < TEST_DURATION_SECONDS:
                if not futures_in_flight:
                    break
                done_futures, pending = ray.wait(futures_in_flight, num_returns=1)
                futures_in_flight = pending

                for fut in done_futures:
                    result = ray.get(fut)
                    actor_idx = future_to_actor_idx.pop(fut, None)
                    submit_ts = submit_time_map.pop(fut)
                    now = time.perf_counter()
                    e2e_ms = (now - submit_ts) * 1000
                    ...
                    if (time.perf_counter() - start_time) < TEST_DURATION_SECONDS and actor_idx is not None:
                        new_actor = actors[actor_idx]
                        img_array = np.array(random.choice(images))
                        new_submit_ts = time.perf_counter()
                        new_fut = new_actor.run_benchmark.remote(batch_size, img_array)
            ...
            if futures_in_flight:
                ...
                now = time.perf_counter()
                for fut, res in zip(futures_in_flight, remaining_results):
                    submit_ts = submit_time_map.pop(fut, now)
                    e2e_ms = (now - submit_ts) * 1000
                    stats["server_latencies"].append(res["latency_ms"])
                    stats["e2e_latencies"].append(e2e_ms)
                    stats["total_requests"] += 1
            ...
            actual_duration = max(time.perf_counter() - start_time, 1e-9)
            server_lats = np.array(stats["server_latencies"])
            e2e_lats = np.array(stats["e2e_latencies"])

            tps = stats["total_images"] / actual_duration
            rps = stats["total_requests"] / actual_duration
            e2e_avg = float(np.mean(e2e_lats)) if len(e2e_lats) else float("nan")
            avg_lat_img = (e2e_avg / batch_size) if batch_size and not math.isnan(e2e_avg) else float("nan")
            ...
            result_rows.append(
                {
                    ...
                    "avg_lat_per_img_ms": avg_lat_img,
                    "qps": rps,
                    "rps": tps,
            ...
            if len(e2e_lats) > 0:
                logger.info(
                    ...
                    f"Avg/img={avg_lat_img if not math.isnan(avg_lat_img) else float('nan'):.2f}ms"
                )
